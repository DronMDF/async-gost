...
void test_imit()
{
	...
	const vector<uint8_t> key = {
		...
		0x2F, 0x79, 0xCD, 0x9E, 0xD2, 0xBC, 0xEF, 0xBD };
	// When
	auto result_context = async_imit(text, key);
	auto result = result_context.get().data;
	// Then
	const vector<uint8_t> expected = { 0x54, 0x8F, 0x73, 0x46 };
	// Тестируем по содержимому expected, в result больше данных
	if (!equal(expected.begin(), expected.end(), result.begin())) {
		throw runtime_error("Имитатор сломался");
	}
	cout << "Имитатор работает хорошо." << endl;
}
